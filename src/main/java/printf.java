...
public class printf {
    ...
    private static void printInfo(double totalIncome, double totalExpense){
        ...
        String totalIncomeStr = String.format("%.1f", totalIncome);
        String totalExpenseStr = String.format("%.2f", totalExpense);
        String averageIncomeStr = String.format("%.2f", totalIncome/31);
        String averageExpenseStr = String.format("%.2f", totalExpense/31);

        System.out.println("Total income = $" + totalIncomeStr + " ($"+ averageIncomeStr + "/day)");
        System.out.printf("Total income = $%.2f ($%.2f/day)%n ", totalIncome, (totalIncome/31));

        System.out.println("Total expenses = $" + totalExpenseStr + "($" + averageExpenseStr +"/day)");
