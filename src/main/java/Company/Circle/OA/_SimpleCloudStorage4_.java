...
public class _SimpleCloudStorage4_ {
    ...
    static Map<String, List<String[]>> trash = new HashMap<>(); 
    ...
    public static void main(String[] args){
        ...
        for (String[] query : queries1){
            ...
            } else if (fun.equals("DELETE_FILES")){
                ans[i] = deleteFiles(query[1]);
            } else if (fun.equals("RESTORE_FILES")){
                ans[i] = restoreFiles(query[1]);
    ...
    private static String deleteFiles(String fileName){
        int ans = 0;
    ...
    private static String restoreFiles(String fileName){
        int ans = 0;
        for (String key : trash.keySet()){
            if (key.startsWith(fileName)) {
                List<String[]> versions = trash.get(key);
                // ans += versions.size();
                ans ++;
